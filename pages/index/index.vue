<template>
  <view class="container">
    <!-- 星座动画组件 -->
    <zodiac-animation
      v-if="showZodiacAnimation"
      :birth-month="userInfo.birthMonth || 1"
      :birth-day="userInfo.birthDay || 1"
    ...
    <!-- 星座背景组件 -->
    <zodiac-background
      ref="zodiacBackground"
      :birth-month="userInfo.birthMonth || 1"
      :birth-day="userInfo.birthDay || 1"
      :show-symbols="true"
      :show-particles="true"
      :show-background-video="true"
      :play-background-music.sync="bgMusicPlaying"
    />

    <!-- 音乐控制按钮 -->
    <view class="music-control">
      <button class="music-btn" @click="toggleBackgroundMusic">
        <text>{{ bgMusicPlaying ? "暂停" : "播放" }}</text>
      </button>
    </view>
    ...
    <!-- #endif -->
    <text class="noData" v-if="msgList.length === 0">没有记录</text>
    <scroll-view
      ...
      :enable-flex="true"
    >
      <!-- <ua-markdown :source="msgList[msgList.length - 1].content" :key="index" /> -->
      <uni-ai-msg ref="msg" :msg="msgList[msgList.length - 1]"></uni-ai-msg>
      ...
      <view v-if="adpid" class="open-ad-btn-box">
        <text style="color: red"
          ...
          位置：/pages/chat/chat.vue 第30行，或全局搜索
          uni-ad-rewarded-video</text
        >
        <!-- <uni-ad-rewarded-video v-show="insufficientScore" :adpid="adpid" @onAdClose="onAdClose"></uni-ad-rewarded-video> -->
      </view>
      <view @click="closeSseChannel" class="stop-responding" v-if="sseIndex"
        >▣ 停止响应</view
      >
    ...
    <view
      class="send-btn-box"
      :title="
        msgList.length && msgList.length % 2 !== 0 ? 'ai正在回复中不能发送' : ''
      "
    >
      <!-- #ifdef H5 -->
      <!-- <text v-if="isWidescreen" class="send-btn-tip">↵ 发送 / shift + ↵ 换行</text> -->
      <!-- #endif -->
      <!-- <button @click="beforeSend" :disabled="inputBoxDisabled || !content" class="send" type="primary">发送</button> -->
    </view>
    <!-- </view> -->
    <!-- </view> -->
...
// 引入星座动画和背景组件
import ZodiacAnimation from "@/components/ZodiacAnimation.vue";
import ZodiacBackground from "@/components/ZodiacBackground.vue";
// 收集所有执行云对象的任务列表
let uniCoTaskList = [];
...
export default {
  ...
  data() {
    return {
      ...
      // 输入框的消息内容
      content:
        '参考用户个人信息和今日天气情况，生成一个今日指引，包括个人信息包括年龄（用户年龄给一个幽默的修饰词）、星座、天气情况、着装建议，谈事情的时间，感情、会客时间等等。让我心理更有依据和信息去完成每日的工作生活。输出内容不要写"玄学"二字。天气如下：',
      // 记录流式响应次数
      sseIndex: 0,
      // 是否启用流式响应模式
      enableStream: true,
  ...
  onShow: function () {
    // 检查是否首次进入
    console.log("检查是否首次进入");
    let isFirstTime = uni.getStorageSync("isFirstTime");
    let hasPlayedOpeningVideo = uni.getStorageSync("hasPlayedOpeningVideo");
    let isNavigating = uni.getStorageSync("isNavigating"); // 防止导航循环

    // 如果正在处理导航，跳过
    if (isNavigating) {
      console.log("已有导航请求正在处理，跳过");
    ...
    if (!isFirstTime) {
      ...
      // 使用setTimeout延迟导航，避免立即执行可能导致的超时问题
      setTimeout(() => {
        uni.reLaunch({
          url: "/pages/guide/guide",
          success: function () {
            console.log("成功跳转到guide页面");
          },
          fail: function (err) {
            ...
            // 设置为默认用户信息并将isFirstTime设为true以跳过引导
            let defaultUserInfo = {
              nickname: "新用户",
              signature: "欢迎使用",
              birthYear: "1996",
      ...
      // 如果没播放过开场视频，跳转到开场视频页面
      uni.navigateTo({
        url: "/pages/opening/index",
        animationType: "none",
      });
      // 设置为已播放开场视频
      uni.setStorageSync("hasPlayedOpeningVideo", true);
    }
  ...
  async mounted() {
    ...
    // 确定使用视频还是SVG（可以根据设备性能或偏好设置）
    // 此处设置根据设备来判断
    const systemInfo = uni.getSystemInfoSync();
    // 高端设备使用视频，低端设备使用SVG
    this.animationType =
      systemInfo.platform === "android" && systemInfo.system.split(" ")[1] < 9
        ? "svg"
        : "video";

    const wetherData = await uni.request({
    ...
    if (wetherData.data.status == 1) {
      ...
      if (data.count > 0) {
        ...
        console.log(formattedDate); // 输出：2025-03-10
        this.content =
          this.content +
          data.lives[0].city +
          "气温" +
    ...
    this.insufficientScore =
      uni.getStorageSync("uni-ai-chat-insufficientScore") || false;
  ...
  methods: {
    ...
    async send() {
      ...
      let task = uniCoTask({
        ...
        success: (res) => {
          ...
          if (
            this.enableStream == false ||
            (this.sseIndex == 0 && (illegal || insufficientScore))
          ) {
            // 将从云端接收到的消息添加到消息列表中
            this.msgList.push({
              // 消息创建时间
    ...
    toggleBackgroundMusic() {
      if (this.$refs.zodiacBackground) {
        ...
        // 不需要手动更新bgMusicPlaying，它会通过.sync自动更新
      }
    },
  },
};
...
.container {
  ...
  min-height: 100vh;
  position: relative;
  background-color: transparent; // 修改为透明，让星座背景显示
}
// .container {
// 	position: relative;
// 	width: 100vw;
